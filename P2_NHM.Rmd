...
#Sale que una variable discreta no es recomendable por el tamaño de la muestra. Esto es porque los valores de las variables son contínuos (tienen decimales), no son discretos.
```
:::
...
#Esta rellenando toda la gráfica, en este caso hasta el 1, es decir, como que del total se ve si hay mas rojo o azul (no por altura sino por cantidad de trozo de color)
```
:::
...
#si tenemos que pasarlo previamente a factor para que pueda determinar que son tres niveles, despues de eso usamos lo mismo y nos sale un plot (usando dodge) que salen de cada cyl los 3 colores que son cada gear (3, 4 o 5)
```
:::
...
#Lo que hace es cambiarlo de forma, es decir,en vez de horizontal en vertical, pero si pones (~ . cut) te da error, por lo que solo puede ser de dos formas.
```
:::
...
Yo creo que el mejor tipo de subplot es o haciendo dodge, porque te pone del mismo valor, de cada variable. Sino tambien el facet_grid pero en horizontal porque te permite comparar de cada grupo la misma posicion. Para mi son las mas claras de ver.
------------------------------------------------------------------------
...
ggplot(data = diamonds, mapping = aes(y = carat, fill = color)) +
  geom_boxplot()
```
:::
...
```{r answer5.1}
#El brewer da tonalidades azules, cada uno presenta colores diferentes.
```
:::
...
ggplot(data = diamonds, mapping = aes(x = carat, fill = cut)) +
+     geom_histogram(position = "dodge", binwidth = 0.3) + 
+     facet_grid(cut ~ .) + labs(subtitle = "Practica 2", caption ="diamantes", tag = "7/11/2025")+ 
+     ggtitle("Diamond weight variation") +
+     theme_bw()

#sirven para poner el subtitulo, abajo más cosas, en la esquina superior... Podríamos poner titulo y mas. 
```
:::
...
```{r answer5.3}
#Para mi, despues de ver la web de los ocho es el bw, o light por paneles (facet_grid)
```
:::
...
```{r answer 6.1}
#yo no he visto nada distinto, solo que la primera al clicar ya se abre con la medida exacta y el otro es más pequeño.

ggsave(filename= "grafic.svg", plot= p, width = 6, height = 4)
...
ggplot(data = economics, mapping= aes(x = date, y = unemploy)) + geom_line(color="Slate Blue")

plot_ec <- ggplot(data = economics, mapping= aes(x = date, y = unemploy)) + geom_line(color="Slate Blue")

plot_ec + labs(title="Desempleados durante los años", x="años", y="personas") + theme_minimal()

plot_final <- plot_ec + labs(title="Desempleados durante los años", x="años", y="personas") + theme_minimal()

ggsave(filename="plot practica2.svg", plot= plot_final, width=6, height=4)

#he ido guardando pero el final es:
ggplot(data = infert, mapping = aes(y = stratum, x = education, fill=education)) + geom_boxplot() + labs (title= "Infertility after Spontaneous and Induced Abortion", x="Education", y="Stratum number") + facet_grid( . ~ induced) + theme_bw() +scale_fill_manual(values=c("#900C3F", "#C70039", "#FF5733"))
...
#voy a escoger el grupo 8
ggplot(data = infert, mapping = aes(y = stratum, x = education)) +
+ geom_boxplot() 

#guardo en plot 1 el plot sin color y separar por 0,1,2
plot1 <- ggplot(data = infert, mapping = aes(y = stratum, x = education)) + geom_boxplot() + labs (title= "Infertility after Spontaneous and Induced Abortion", x="Education", y="Stratum number")

final_plot<-plot1 + facet_grid( . ~ induced) + theme_bw() + scale_fill_manual(values = c("#900C3F", "#C70039", "#FF5733"))



ggplot(data = infert, mapping = aes(y = stratum, x = education)) + geom_boxplot( colours()) + labs (title= "Infertility after Spontaneous and Induced Abortion", x="Education", y="Stratum number") + facet_grid( . ~ induced) + theme_bw() + scale_fill_manual(values = c("#900C3F", "#C70039", "#FF5733"))
...
```{r analyse_dataset}
data <- read.csv("https://raw.githubusercontent.com/marta-coronado/TAB-data-figs/refs/heads/main/P2_datasets/chap03q15VasopressinVoles.csv")
View(data)
ggplot(data=data, mapping=aes(x=treatment, y= percent, colour=percent)) + geom_point() 

#en base a este dataset (6) no hay mucha información, solo sobre el tipo de tratamiento y el porcentaje en el que se encuentra el receptor vasopresina 1 (V1). Como vemos, el resultado concuerda con la hipótesis, como más cantidad de V1 más monogamo,por ello los enhanced (tratados con incremento de V1) tienen comportamientos más monógamos. Los control, el porcentaje es variado, por lo tanto, habrá un poco de todo, pero todos los tratados tienen un porcentaje alto y todos son monógamos. Debemos también tener en cuenta como dicen los autores que no solo depende de un factor, ya que es un comportamiento sexual complejo.
```
