...
``` There s a warning message because the attribute shape or size are not present in the dataset therefore we cannot map them.
:::
...
```{r positions-ggplot2 answer2.1}
 It is filling the columns instead of separating them side to side making the plot maybe less clear than when using the dodge function. 
```
:::
...
``` ggplot(data = mtcars, mapping = aes(x=cyl_f, fill = gear_f) + geom_bar(position ="fill")
:::
...
The layout of the graph changes switching the position of the cut of the diamond above the number of carats.
```
:::

Which is the best subplot configuration to compare the distributions and
why?
I think the second configuration is better than the first because we can see the data and the differences between them more clearly than in the first one.
...
Cde for filling the plot green
ggplot(data = diamonds, mapping = aes(y = carat, x = color)) +
...
```{r answer5.1}
I would use scale_fill_viridis_d() because it is in blac and whit, it is friendly to use for colorblind people
```
:::
...
::: {style="background-color:#F0F0F0"}
##### <i class="fas fa-comment-dots"></i> Answer:
We use this commands to add more text to our plot. Subtitle appears under the main title, tag and caption in the corners.

```{r answer5.2}
ggplot(data = diamonds, mapping = aes(x = carat, fill = cut)) +
  geom_histogram(position = "dodge", binwidth = 0.3) + 
  facet_grid(cut ~ .) + labs(subtitle = "TAB Practical 2", caption = "Diamonds", tag = "Figure 1")
```
:::
...
```{r answer5.3}
I would say that the theme_minimal for the simplest visualisation of data with no distractions.
```
:::
...
print(p)
ggsave(
  filename = "unemployment_over_time.png",
  plot = p,
  width = 8,
  height = 5,
  dpi = 300
...
print(p)
ggsave(
  filename = "toothgrowth_vitaminC.png",
  plot = p,
  width = 7,
  height = 5,
  dpi = 300
...
summary(data)
library(ggplot2)
class(data$gestationalAgeWks)
data$gestationalAgeWks <- cut(
  data$gestationalAgeWks,
  breaks = c(0, 34, 35, 36, 38),
  labels = c("Early", "Mid", "Late", "Very Late")
 )
ggplot(data, aes(x = gestationalAgeWks, y = biparietalDiamMm)) +
  geom_boxplot(fill = "lightblue") +
  labs(
    title = "BPD Distribution by Gestational Week",
    x = "Gestational Age (weeks)",
    y = "Biparietal Diameter (mm)"
  ) +
  theme_minimal()
