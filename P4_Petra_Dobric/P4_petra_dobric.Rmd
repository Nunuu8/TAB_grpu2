...
dir.create("Practical 4")
setwd("Practical 4")
dir.create("data")
dir.create("data/bileductCancer")
list.dirs()
```
...
url_bile_duct <- "http://duffel.rail.bio/recount/v2/TCGA/rse_gene_bile_duct.Rdata"
destfile <- "rse_gene_bile_duct.Rdata"

# Download if not exists
if (!file.exists(destfile)) {
  download.file(url_bile_duct, destfile = destfile, mode = "wb")
}

# Load data
load(destfile)
setwd("Practical 4/data/bileductCancer")
system("wget http://duffel.rail.bio/recount/v2/TCGA/rse_gene_bile_duct.Rdata ")
...
load(file = "rse_gene_bile_duct.Rdata")
```
...
For each gene we can see the gene id, bp length and the symbol.
</div>
...
<div style="background-color:#F0F0F0">
##### &emsp;<i class="fas fa-comment-dots"></i> Answer:
There are totally 45 patients, 36 of them are in an early stage and 9 in a late stage of cancer. We have information about their phenotype as well as information on 58037 genes.
...
<div style="background-color:#F0F0F0">
##### &emsp;<i class="fas fa-comment-dots"></i> Answer:
When using the function class, we can tell that res is "DESeqResults" object which has statistical information on the expsression of a gene.
</div>
...
<div style="background-color:#F0F0F0">
##### &emsp;<i class="fas fa-comment-dots"></i> Answer:
After running the code we can see that there are 348 underexpressed and 149 overexpressed genes. By substracting those number from the total number of genes we get a number of non differentially expressed genes.
</div>
...
```{r ggplot1}
library(ggplot2)
ggplot(res_tb, aes(x = log2FoldChange, y = -log10(padj))) + geom_point()
...
```{r ggplot2}
ggplot(res_tb, aes(x = log2FoldChange, y = -log10(padj), color = filter)) +
  geom_point(alpha = 0.6, size = 1.5) +
  theme_minimal() +
  scale_color_manual(values = c("grey", "red"), 
                     labels = c("Not significant", "Significant")) +
  labs(title = "Volcano polt: Bile duct cancer",
...
ggplot(res_tb, aes(x = log2FoldChange, y = -log10(padj), color = filter)) +
  ...
  scale_color_manual(values = c("grey", "red"), 
                     labels = c("Not significant", "Significant")) +
  labs(title = "Volcano plot: Bile duct cancer",
       x = "Effect size (log2 fold change)",
       y = "-Log10( adjusted p-value)",
       color = "Significance") +
  geom_label_repel(data = res_tb[res_tb$filter == TRUE, ], 
...
##### &emsp;<i class="fas fa-comment-dots"></i> Answer:
```{r}
ego_mf <- enrichGO(gene = signif_genes,
                   keyType = "SYMBOL",
                   OrgDb = org.Hs.eg.db,
                   ont = "MF",
                   pAdjustMethod = "BH",
...
dotplot(ego_mf, showCategory=15) +
  theme(axis.text.y = element_text(size=8)) +
  ggtitle("GO Enrichment-MF")


ego_cc <- enrichGO(gene = signif_genes,
                   keyType = "SYMBOL",
                   OrgDb = org.Hs.eg.db,
                   ont = "CC",
                   pAdjustMethod = "BH",
...
keggResult = enrichKEGG(gene = dedup_ids$ENTREZID,
              ...
              keyType = "ncbi-geneid")
head(keggResult)
dotplot(keggResult, showCategory =15)
ggsave("dotplot.png")
...
<div style="background-color:#F0F0F0">
##### &emsp;<i class="fas fa-comment-dots"></i> Answer:
The plot shows a single significant pathway which is olfactory transduction. The size of the dot indicates that 6 genes are included in this process. The analysis was done with adjusted, "more relaxed" thresholds and still shows only one significant pathway which suggests a low signal-to-noise ratio within the dataset.
</div>
...
# Create a named vector with log2FC values
gene_data <- signif_res$log2FoldChange
names(gene_data) <- signif_res$gene
...
#The pathway analysis generates an image showing the olfactory transduction pathway. It show us in detail how odorant molecules bind to receptors and trigger a response.
```
